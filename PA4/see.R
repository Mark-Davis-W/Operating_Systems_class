...
plot.procPages <- function (pl, low=0, high=1e25) { 
    ...
    plot(c(), xlab='time', ylab='memory', ylim=c(0,max(toplot$page)*256), xlim=c(toplot$time[[1]],toplot$time[[length(toplot$time)]]))
...
lines.procPages <- function (pl, low=0, high=1e25) { 
	...
	if (!is.na(refs[[page+1]])) { 
	    ...
	    ybottom[[length(ybottom)+1]] = page*256
	    ytop[[length(ytop)+1]] = (page+1)*256
	    if (oldcomm=='in') { 
